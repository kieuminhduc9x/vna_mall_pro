...
export function phoneValidatorNew (rule, value, callback) {
  if (value) {
    const reg1 = new RegExp('^[0+][0-9]+$', 'g')
    if (reg1.test(value) === false) {
      callback(new Error(' Số điện thoại nhập sai định dạng'))
      ...
      if (value.toString().trim().length === 10) {
        ...
        callback(new Error('Số điện thoại gồm 10 ký tự'))
      }
    }
  }
  callback()
...
export function numberRequired60 (rule, value, callback) {
  if (value) {
    const reg = new RegExp('^-?[0-9]+$', 'g')
    if (reg.test(value) === false) {
      callback(new Error(' Chỉ cho phép nhập ký tự số'))
    } else if (Number(value) > 60) {
      callback(new Error('Không nhập quá 60 phút'))
    } else {
      callback()
    }
  }
...
export function validatePrice (rule, value, callback) {
  const priceMax = rule.record.priceMax
  const priceMin = rule.record.priceMin
  console.log(priceMin, priceMax)
  if (value) {
    if (priceMin > priceMax) {
      callback(new Error(' Giá sàn phải nhỏ hơn hoặc bằng giá trần'))
    } else {
      callback()
    }
  }
...
export function validatePriceMax (rule, value, callback) {
  const priceMax = rule.record.priceMax
  const priceMin = rule.record.priceMin
  console.log(priceMin, priceMax)
  if (value) {
    if (priceMin > priceMax) {
      callback(new Error(' Giá trần phải lớn hơn hoặc bằng giá sàn'))
    } else {
      callback()
    }
  }
...
// validate các field khi nhập mã
